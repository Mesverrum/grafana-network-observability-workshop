...
**Student SNMP story is the colocated 3-site Clos**, not the mock campus names and not the leftover OTLP generator. Infinity mocks (PRTG / Check Point / EdgeConnect / Meraki) are a **separate** Lab 5 API. Those hostnames are not in SNMP.

**Synthetics workflow:** one public VIP for every stack. Lab 2: traceroute + TCP from Oregon or N. Virginia. Lab 4: they add a **public Singapore** probe on that same IP (vantage, then an alert). They never select a private probe. Hairpin / GA origin toggle is optional facilitator `curl` only — [`facilitator/singapore-fault.md`](facilitator/singapore-fault.md), [`facilitator/hairpin.md`](facilitator/hairpin.md).
...
3. Host the mock API where Cloud can GET it (Lab 5 Infinity only):
