...
Facilitator wrote a **target hostname** on the board. Use that. Do not invent a customer production VIP.
...
5. Target: the hostname on the board.
...
3. Target: hostname **and port** from the board (example `lab.example.com:443`).
...
Write down today's hop count and a ballpark duration. You will need that after the fault.

## You are done when

Both checks have a result, and you know which probe you used.
